...
类就是对象的种类。对象拥有什么特性是由类决定的。如果一个对象属于某个类，就可称它为`xx类的对象`或`xx类的实例`。在上下文允许的情况下，“类的实例”通常简称为“实例”。在类上调用的方法，方法调用的接收者是类，这类方法称为类方法；在对象上，即类的实例上，调用的方法，方法调用的接收者是对象，这类方法称为实例方法。
